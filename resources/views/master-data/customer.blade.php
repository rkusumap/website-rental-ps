            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-12">
                        ...
                        >
                        <div class="invalid-feedback" id="error-name_customer"></div>
                    </div>
                </div>

                <div class="form-group row">
                    <label class="col-sm-12 ">No. Telp Customer</label>
                    <div class="col-sm-12">
                        <input type="text" name="phone_customer" id="phone_customer" class="form-control"
                        >
                        <div class="invalid-feedback" id="error-phone_customer"></div>
                    </div>
                </div>
            </div>
...
<script>
  $(function () {
    ...
    $(document).on('click', '.btn-tambah', function(){
        ...
        $('#formmodallabel').html('Tambah Customer');
        $('.btn-simpan').val('tambah')
    })

    $(document).on('click', '.btn-edit', function(){
        $('#formmodal').modal('show');
        $('#formmodallabel').html('Edit Customer');
        $('.btn-simpan').val('update')
        var id = $(this).data('id');
        // Send an AJAX request
        $.ajax({
            url: '/customer/'+id, // Replace with your route
            type: 'get',
            success: function (response) {
                // Handle success response
                $('#name_customer').val(response.data.name_customer)
                $('#phone_customer').val(response.data.phone_customer)
                $('.btn-simpan').data('id', response.data.id_customer)
                $('#formmodal').modal('show');
                $('.btn-simpan').val('update')
            },
