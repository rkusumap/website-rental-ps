        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group row">
                    <label class="col-sm-12 ">Nama Merek</label>
                    <div class="col-sm-12">
                        <input type="text" name="name_brand" id="name_brand" class="form-control"
                        placeholder="Contoh : Indofood">
                        <div class="invalid-feedback" id="error-name_brand"></div>
                    </div>
                </div>
            </div>
...
<script>
  $(function () {
    ...
    $(document).on('click', '.btn-tambah', function(){
        ...
        $('#formmodallabel').html('Tambah Brand');
        $('.btn-simpan').val('tambah')
    })

    $(document).on('click', '.btn-edit', function(){
        $('#formmodal').modal('show');
        $('#formmodallabel').html('Edit Brand');
        $('.btn-simpan').val('update')
        var id = $(this).data('id');
        // Send an AJAX request
        $.ajax({
            url: '/brand/'+id, // Replace with your route
            type: 'get',
            success: function (response) {
                // Handle success response
                $('#name_brand').val(response.data.name_brand)
                $('#formmodal').modal('show');
                $('.btn-simpan').val('update')
                $('.btn-simpan').data('id', response.data.id_brand)
            },
